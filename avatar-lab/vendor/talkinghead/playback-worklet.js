class PlaybackWorklet extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    // Playback speed multiplier (1 = realtime). Applied at intake by linearly
    // resampling each queued chunk, so the existing process() loop is untouched.
    this._speed = Math.min(2, Math.max(0.5, Number(options?.processorOptions?.speed) || 1));
  ...
  handleMessage(event) {
    ...
    // Update playback speed at runtime (applies to chunks queued from now on)
    if (type === "config-speed" && data && typeof data === "object") {
      const s = Number(data.speed);
      if (isFinite(s)) this._speed = Math.min(2, Math.max(0.5, s || 1));
      return;
    }
    ...
    if (type === "audioData" && data instanceof ArrayBuffer) {
      ...
      if (this._state === PlaybackWorklet.FSM.PLAYING) {
        ...
        // When speed != 1, linearly resample at intake: chunk duration scales
        // by 1/speed. Per-chunk edge error is at most one sample (~0.02 ms).
        let chunk = new Int16Array(data);
        if (this._speed !== 1 && chunk.length > 1) {
          const outLen = Math.max(2, Math.round(chunk.length / this._speed));
          const resampled = new Int16Array(outLen);
          const last = chunk.length - 1;
          for (let i = 0; i < outLen; i++) {
            const pos = Math.min(last, i * this._speed);
            const i0 = pos | 0;
            const frac = pos - i0;
            const a = chunk[i0];
            const b = i0 < last ? chunk[i0 + 1] : a;
